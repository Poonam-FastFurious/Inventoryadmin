...
const useAdminStore = create((set) => ({
  ...
  login: async (email, password) => {
    try {
      ...
      if (res.data && res.data.success && res.data.data?.accessToken) {
        ...
        const decoded = jwtDecode(res.data.data.refreshToken);
        const adminId = decoded._id || decoded.id; // Ensure correct key based on your token

        await useAdminStore.getState().getAdminDetails(adminId); // optional
  ...
  getDashboardHeaderData: async (userId) => {
    try {
      ...
      const res = await axios.get(
        `${Baseurl}dashboard/dashboard-header?userId=${userId}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
